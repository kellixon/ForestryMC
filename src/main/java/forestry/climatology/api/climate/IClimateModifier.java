...
/**
 * This modifier is used by {@link IClimateLogic}s.
 */
public interface IClimateModifier {

	/**
	 * Is called every 20 ticks by the {@link IClimateLogic} to change its climate state.
	 *
	 * @param logic    the climate container
	 * @param newState the new climate state
	 * @param oldState the climate static of the previous modification.
	 * @param data     A {@link NBTTagCompound} that can be used to save custom data.
	 * @return a modify newState or the newState.
	 */
	default IClimateState modifyTarget(IClimateLogic logic, IClimateState newState, IClimateState oldState, NBTTagCompound data) {
		return newState;
	}

	@SideOnly(Side.CLIENT)
	default void addInformation(IClimateLogic container, NBTTagCompound nbtData, ClimateType type, List<String> lines) {
	}
